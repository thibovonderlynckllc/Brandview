...
# Final video conversion script for iOS Safari compatibility
# Handles portrait videos and ensures dimensions are divisible by 2
...
# Function to convert video with proper scaling
convert_video() {
    local input_file="$1"
    ...
    # Use a more flexible scaling approach that ensures dimensions are divisible by 2
    ffmpeg -i "$input_file" \
           -c:v libx264 \
           -preset medium \
           -crf 23 \
...
# Convert each video individually
